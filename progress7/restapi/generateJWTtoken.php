...
$mongoModel = new MongoDatabaseClass( $restConfig );
...
if (isset($_GET['deviceId'])) {
	...
	//$devicesfilenames = scandir( ($restConfig->apacheAbsolutePath) . "devices" );
	//$restConfig->permissionHardwareList=$devicesfilenames; //replace all hardcoded devices to real connection links
	...
	foreach ( $permissionHardwareList as $compareHardwareElement ) {              
		...
		if ($compareHardware == $inputHardware) {
			...
			$mongoModel->bindJWTtokenToConnectionDevice(  $inputHardware , $token  ); //$token 
			...
			//$myfile = fopen($restConfig->apacheAbsolutePath . 'devices/' . $inputHardware . ".txt", "w");
