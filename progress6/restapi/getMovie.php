...
if (isset($_GET['jwtstring'])  &&  isset($_GET['deviceId'])) {
	...
	$devicesfilenames = scandir( ($movieRestConfig->apacheAbsolutePath) . "devices" );
	$movieRestConfig->permissionHardwareList=$devicesfilenames; //replace all hardcoded devices to real connection links
	...
	for( $deviceSearchIndex = 0; $deviceSearchIndex < count($devicesfilenames) ;  $deviceSearchIndex++ ){
		...
		if ($compareHardware2 == $inputHardware) {
			...
			$myfile = fopen($movieRestConfig->apacheAbsolutePath . 'devices/' . $inputHardware . ".txt", "r");
			$serverJWTTOken = fread($myfile, filesize($movieRestConfig->apacheAbsolutePath . 'devices/' . $inputHardware . ".txt"));
			fclose($myfile);
